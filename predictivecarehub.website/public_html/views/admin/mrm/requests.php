                    ...
                    <div class="container w-full md:w-4/5 xl:w-3/5" style="width:100%; padding-top: 1em;  padding-bottom: 1em;">
                        ...
                        <div class="p-8 mt-6 lg:mt-0 rounded shadow bg-white w-full">
                            ...
                            <table id="request_patients" class="stripe hover" style="width:100%; padding-top: 1em;  padding-bottom: 1em;">
                                ...
                                <tbody>
                                    <?php
                                    $sql = "SELECT id, patient_id, patient_name, request_type, doctor_name, doctor_id, seen, createdAt FROM medical_records WHERE archived = 0 AND approved = 1 AND uploaded = 0";
                                    $result = mysqli_query($conn, $sql);

                                    if (mysqli_num_rows($result) > 0) {
                                        while ($row = mysqli_fetch_assoc($result)) {
                                            $seen = $row['seen'] == 1 ? '' : 'hidden';
                                            ...
                                            echo "<tr>
                                        <td><span class='text-sm bg-green-300 rounded-full aspect-square p-2 " . $seen . "'>New</span> " . e($row['patient_id']) . "</td>
                                        ...
                                        <td>" . e($row['createdAt']) . "</td>";
                                            ...
                                            <button class='group upload_medical' data-id='" . e($row['id']) . "' title='Upload'>
                                                <i class='fas fa-solid fa-upload text-blue-600'></i> Upload
                                            </button>
                                            <button class='group delete_medical' data-delete-type='delete_medical' data-delete-id='" . e($row['id']) . "' title='Delete'>
                                                <i class='fas fa-solid fa-trash text-red-600 group-hover:text-red-500'></i> Delete
                                            </button>
                                        </td>
                                    </tr>";
                                            $updateStmt = mysqli_prepare($conn, "UPDATE medical_records SET seen=2 WHERE id=?");
