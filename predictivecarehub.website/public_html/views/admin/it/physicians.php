...
<body class="bg-gray-100 font-family-karla flex">
    ...
    <div class="relative w-full flex flex-col h-screen overflow-y-hidden relative">
        ...
        <div class="w-full h-screen overflow-x-hidden border-t flex flex-col">
            <main class="w-full flex-grow p-6 mb-24">
                <h1 class="text-3xl text-black pb-6">Physicians</h1>
                <button id="add_physician_btn" class="w-auto px-5 bg-gray-950 text-white hover:bg-gray-900 text-gray-900 font-semibold py-2 mt-5 rounded-br-lg rounded-bl-lg rounded-tr-lg shadow-lg hover:shadow-xl flex items-center justify-center">
                    <i class="fas fa-plus mr-3"></i> Add Physician
                </button>
                <div class="w-full mt-6">
                    <p class="text-xl pb-3 flex items-center">
                        <i class="fas fa-list mr-3"></i> List of Physicians
                    </p>
                            ...
                            <table id="patients" class="stripe hover" style="width:100%; padding-top: 1em;  padding-bottom: 1em;">
                                <thead>
                                    <tr>
                                        ...
                                        <th data-priority="3" class="text-left">Role</th>
                                        <th data-priority="1" class="text-left">Actions</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <?php
                                    $sql = "SELECT physician_id, physician_name, physician_role FROM physicians";
                                    $result = mysqli_query($conn, $sql);

                                    if (mysqli_num_rows($result) > 0) {
                                        while ($row = mysqli_fetch_assoc($result)) {
                                            echo "<tr>
                                        <td>" . e($row['physician_name']) . "</td>
                                        <td>" . e($row['physician_role']) . "</td>";
                                            echo "<td class='flex items-center flex-wrap sm:flex-nowrap gap-6'>
                                            <button class='group edit_physician' data-id=" . e($row['physician_id']) . " data-name='" . e($row['physician_name']) . "' data-role='" . e($row['physician_role']) . "' title='Edit'>
                                            ...
                                            <button class='group delete_physician' data-delete-type='delete_physician' data-delete-id=" . e($row['physician_id']) . " title='Delete'>
    ...
    <?php include 'partials/add_physicians_modal.php'; ?>
    <?php include 'partials/delete_modal.php'; ?>
    <?php include 'partials/loading.php'; ?>
    <?php include 'partials/edit_physician_modal.php'; ?>
